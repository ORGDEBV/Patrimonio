...
PrimeFaces.ajax = {
        ...
        updateElement: function(f, c, e) {
            if (f.indexOf(PrimeFaces.VIEW_STATE) !== -1) {
                ...
                if (f.indexOf(PrimeFaces.CLIENT_WINDOW) !== -1) {
                    PrimeFaces.ajax.Utils.updateFormStateInput(PrimeFaces.CLIENT_WINDOW, c, e)
                } else {
    ...
    Request: {
        ...
        send: function(e) {
            ...
            var n = (e.global === true || e.global === undefined) ? true : false,
                b = null,
                f = null;
            if (e.onstart) {
            ...
            PrimeFaces.ajax.Request.addParam(g, PrimeFaces.PARTIAL_REQUEST_PARAM, true, l);
            PrimeFaces.ajax.Request.addParam(g, PrimeFaces.PARTIAL_SOURCE_PARAM, f, l);
            if (e.resetValues) {
                PrimeFaces.ajax.Request.addParam(g, PrimeFaces.RESET_VALUES_PARAM, true, l)
            }
            if (e.ignoreAutoUpdate) {
                PrimeFaces.ajax.Request.addParam(g, PrimeFaces.IGNORE_AUTO_UPDATE_PARAM, true, l)
            }
            ...
            if (a !== "@none") {
                PrimeFaces.ajax.Request.addParam(g, PrimeFaces.PARTIAL_PROCESS_PARAM, a, l)
            }
            ...
            if (d.length > 0) {
                PrimeFaces.ajax.Request.addParam(g, PrimeFaces.PARTIAL_UPDATE_PARAM, d.join(" "), l)
            }
            if (e.event) {
                PrimeFaces.ajax.Request.addParam(g, PrimeFaces.BEHAVIOR_EVENT_PARAM, e.event, l);
                var k = e.event;
                ...
                PrimeFaces.ajax.Request.addParam(g, PrimeFaces.PARTIAL_EVENT_PARAM, k, l)
            } else {
            ...
            if (e.partialSubmit && a.indexOf("@all") === -1) {
                ...
                if (!o) {
                    PrimeFaces.ajax.Request.addParamFromInput(g, PrimeFaces.VIEW_STATE, b, l);
                    PrimeFaces.ajax.Request.addParamFromInput(g, PrimeFaces.CLIENT_WINDOW, b, l);
                    PrimeFaces.ajax.Request.addParamFromInput(g, "dsPostWindowId", b, l);
                    PrimeFaces.ajax.Request.addParamFromInput(g, "dspwid", b, l)
                }
            } else {
    ...
    Response: {
        ...
        handleReFocus: function(b) {
            if (PrimeFaces.customFocus === false && b && b !== $(document.activeElement).attr("id")) {
...
PrimeFaces.dialog.DialogHandler = {
    openDialog: function(e) {
        ...
        var h = e.sourceComponentId.replace(/:/g, "_") + "_dlgwidget",
            d = $('<div id="' + i + '" class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-shadow ui-hidden-container ui-overlay-hidden" data-pfdlgcid="' + e.pfdlgcid + '" data-widgetvar="' + h + '"></div>').append('<div class="ui-dialog-titlebar ui-widget-header ui-helper-clearfix ui-corner-top"><span class="ui-dialog-title"></span></div>');
        ...
        var c = d.find("iframe"),
            b = e.url.indexOf("?") === -1 ? "?" : "&",
            a = e.url + b + "pfdlgcid=" + e.pfdlgcid,
            g = e.options.contentWidth || 640;
        c.width(g);
        c.on("load", function() {
            var o = $(this),
                k = o.contents().find("title"),
                n = false;
            if (e.options.headerElement) {
                var m = PrimeFaces.escapeClientId(e.options.headerElement),
                    j = c.contents().find(m);
                if (j.length) {
                    k = j;
                    n = true
            ...
            if (!o.data("initialized")) {
                PrimeFaces.cw("DynamicDialog", h, {
                    ...
                    onHide: function() {
                        var r = this,
                            q = this.content.children("iframe");
                        if (q.get(0).contentWindow.PrimeFaces) {
                            this.destroyIntervalId = setInterval(function() {
                                if (q.get(0).contentWindow.PrimeFaces.ajax.Queue.isEmpty()) {
            ...
            if (e.options.contentHeight) {
                ...
                l = o.get(0).contentWindow.document.body.scrollHeight + (PrimeFaces.env.browser.webkit ? 5 : 20)
            }
            o.css("height", l);
    ...
    bindKeyEvents: function() {
        ...
        if (this.cfg.queryEvent !== "enter") {
            this.input.on("input propertychange", function(d) {
                ...
                if (PrimeFaces.isIE(8) && (a.itemClick || d.originalEvent.propertyName !== "value")) {
                    a.itemClick = false;
                    return
                }
                var c = a.input.val();
...
PrimeFaces.widget.ColumnToggler = PrimeFaces.widget.DeferredWidget.extend({
    ...
    render: function() {
        ...
        for (var b = 0; b < this.columns.length; b++) {
            var c = this.columns.eq(b),
                d = c.hasClass("ui-helper-hidden"),
                e = d ? "ui-chkbox-box ui-widget ui-corner-all ui-state-default" : "ui-chkbox-box ui-widget ui-corner-all ui-state-default ui-state-active",
                a = (d) ? "ui-chkbox-icon ui-icon ui-icon-blank" : "ui-chkbox-icon ui-icon ui-icon-check";
    ...
    check: function(b) {
        ...
        var a = $(document.getElementById(b.closest("li.ui-columntoggler-item").data("column"))).index() + 1,
            c = this.thead.children("tr").find("th:nth-child(" + a + ")");
        c.removeClass("ui-helper-hidden");
        $(PrimeFaces.escapeClientId(c.attr("id") + "_clone")).removeClass("ui-helper-hidden");
    ...
    uncheck: function(b) {
        ...
        $(PrimeFaces.escapeClientId(c.attr("id") + "_clone")).addClass("ui-helper-hidden");
...
PrimeFaces.widget.DataTable = PrimeFaces.widget.DeferredWidget.extend({
    ...
    initReflow: function() {
        ...
        for (var b = 0; b < a.length; b++) {
            ...
            this.tbody.find("> tr > td:nth-child(" + (b + 1) + ")").prepend('<span class="ui-column-title">' + d + "</span>")
        }
    },
    ...
    resize: function(a, k) {
        ...
        var e = c.next();
        if (this.cfg.liveResize) {
            j = c.outerWidth() - (a.pageX - c.offset().left), d = (c.width() - j), f = (e.width() + j)
        } else {
            j = (k.position.left - k.originalPosition.left), d = (c.width() + j), f = (e.width() - j)
        }
        if ((d > 15 && f > 15) || (m && d > 15)) {
            if (m) {
                n.width(n.width() + j);
            ...
            if (this.cfg.scrollable) {
                var i = this.theadClone.parent(),
                    l = c.index();
                if (m) {
                    var h = this;
                    i.width(i.width() + j);
                    this.footerTable.width(this.footerTable.width() + j);
                    setTimeout(function() {
                        if (h.hasColumnGroup) {
                            h.theadClone.find("> tr:first").children("th").eq(l).width(d);
                            h.footerTable.find("> tfoot > tr:first").children("th").eq(l).width(d)
                        } else {
                            h.theadClone.find(PrimeFaces.escapeClientId(c.attr("id") + "_clone")).width(d);
                            h.footerCols.eq(l).width(d)
                        }
                    }, 1)
                } else {
                    if (this.hasColumnGroup) {
                        this.theadClone.find("> tr:first").children("th").eq(l).width(d);
                        this.theadClone.find("> tr:first").children("th").eq(l + 1).width(f);
                        this.footerTable.find("> tfoot > tr:first").children("th").eq(l).width(d);
                        this.footerTable.find("> tfoot > tr:first").children("th").eq(l + 1).width(f)
                    } else {
...
PrimeFaces.widget.FrozenDataTable = PrimeFaces.widget.DataTable.extend({
    ...
    resize: function(p, m) {
        ...
        var g = r.next();
        var l = r.index(),
            b = r.hasClass("ui-frozen-column-last");
        if (this.cfg.liveResize) {
            i = r.outerWidth() - (p.pageX - r.offset().left), j = (r.width() - i), n = (g.width() + i)
        } else {
            i = (m.position.left - m.originalPosition.left), j = (r.width() + i), n = (g.width() - i)
        }
        var e = (c && j > 15) || (b ? (j > 15) : (j > 15 && n > 15));
        if (e) {
            var h = r.hasClass("ui-frozen-column"),
                k = h ? this.frozenTheadClone : this.scrollTheadClone,
                a = h ? this.frozenThead.parent() : this.scrollThead.parent(),
                d = k.parent(),
                u = h ? this.frozenFooterCols : this.scrollFooterCols,
                t = h ? this.frozenFooterTable : this.scrollFooterTable,
                f = this;
            if (c) {
                if (b) {
                ...
                setTimeout(function() {
                    ...
                    if (f.hasColumnGroup) {
                        ...
                        t.find("> tfoot > tr:first").children("th").eq(l).width(j)
                    } else {
                        k.find(PrimeFaces.escapeClientId(r.attr("id") + "_clone")).width(j);
                        u.eq(l).width(j)
                    }
                }, 1)
            } else {
                ...
                if (this.hasColumnGroup) {
                    k.find("> tr:first").children("th").eq(l).width(j);
                    k.find("> tr:first").children("th").eq(l + 1).width(n);
                    t.find("> tfoot > tr:first").children("th").eq(l).width(j);
                    t.find("> tfoot > tr:first").children("th").eq(l + 1).width(n)
                } else {
                    k.find(PrimeFaces.escapeClientId(r.attr("id") + "_clone")).width(j);
                    k.find(PrimeFaces.escapeClientId(g.attr("id") + "_clone")).width(n);
                    if (u.length > 0) {
                        var s = u.eq(l),
                            q = s.next();
                        s.width(j);
...
PrimeFaces.widget.SelectCheckboxMenu = PrimeFaces.widget.BaseWidget.extend({
    ...
    renderItems: function() {
        ...
        this.inputs.each(function() {
            var j = $(this),
                ...
                k = j.is(":checked"),
                g = j.attr("title"),
                e = "ui-chkbox-box ui-widget ui-corner-all ui-state-default",
                h = "ui-selectcheckboxmenu-item ui-selectcheckboxmenu-list-item ui-corner-all",
                b = j.data("escaped");
            if (d) {
                e += " ui-state-disabled"
            }
            ...
            var f = k ? "ui-chkbox-icon ui-icon ui-icon-check" : "ui-chkbox-icon ui-icon ui-icon-blank",
                h = k ? h + " ui-selectcheckboxmenu-checked" : h + " ui-selectcheckboxmenu-unchecked";
            var l = $('<li class="' + h + '"></li>');
    ...
    checkAll: function() {
        var a = this.itemContainer.children("li.ui-selectcheckboxmenu-item").filter(":visible"),
            b = this;
        a.each(function() {
...
PrimeFaces.widget.ContextMenu = PrimeFaces.widget.TieredMenu.extend({
    ...
    bindDataTable: function() {
        var a = this.jqTargetId + " tbody.ui-datatable-data > tr.ui-widget-content:not(.ui-datatable-empty-message)",
            b = this.cfg.event + ".datatable",
            c = this;
        $(document).off(b, a).on(b, a, null, function(h) {
            ...
            if (f.cfg.selectionMode && i.hasClass("ui-datatable-selectable")) {
                ...
                if (f.cfg.editMode === "cell") {
                    var g = $(h.target),
                        d = g.is("td.ui-editable-column") ? g : g.parents("td.ui-editable-column:first");
    ...
    bindTreeTable: function() {
        var b = this.jqTargetId + " .ui-treetable-data > " + (this.cfg.nodeType ? "tr.ui-treetable-selectable-node." + this.cfg.nodeType : "tr.ui-treetable-selectable-node"),
            c = this.cfg.event + ".treetable",
            a = this;
        $(document).off(c, b).on(c, b, null, function(d) {
            PrimeFaces.widgets[a.cfg.targetWidgetVar].onRowRightClick(d, $(this));
            a.show(d)
        })
    },
    bindTree: function() {
        var b = this.jqTargetId + " .ui-tree-selectable",
            a = this.cfg.nodeType ? this.cfg.event + ".treenode." + this.cfg.nodeType : this.cfg.event + ".treenode",
            d = this.cfg.event + ".tree",
            c = this;
        $(document).off(a, b).on(a, b, null, function(g) {
            var f = $(this);
            if (c.cfg.nodeType === undefined || f.parent().data("nodetype") === c.cfg.nodeType) {
                PrimeFaces.widgets[c.cfg.targetWidgetVar].nodeRightClick(g, f);
                c.show(g)
            }
        });
        $(document).off(d, this.jqTargetId).on(d, this.jqTargetId, null, function(f) {
            if (PrimeFaces.widgets[c.cfg.targetWidgetVar].isEmpty()) {
                c.show(f)
            }
        })
    },
...
PrimeFaces.widget.OrderList = PrimeFaces.widget.BaseWidget.extend({
    ...
    moveUp: function() {
        var b = this,
            d = this.items.filter(".ui-state-highlight"),
            c = d.length,
            a = 0;
        d.each(function() {
            var e = $(this);
    ...
    moveDown: function() {
        var b = this,
            d = $(this.items.filter(".ui-state-highlight").get().reverse()),
            c = d.length,
            a = 0;
        d.each(function() {
            var e = $(this);
...
PrimeFaces.widget.Paginator = PrimeFaces.widget.BaseWidget.extend({
    ...
    bindPageLinkEvents: function() {
        var a = this;
        this.pagesContainer.children(".ui-paginator-page").on("click.paginator", function(c) {
            var b = $(this);
    ...
    updatePageLinks: function() {
        var a, b, k, h = $(document.activeElement),
            c, d;
...
PrimeFaces.widget.TabView = PrimeFaces.widget.DeferredWidget.extend({
    ...
    remove: function(b) {
        var e = this.navContainer.children().eq(b),
            a = this.panelContainer.children().eq(b);
        e.remove();
        a.remove();
    ...
    bindKeyEvents: function() {
        ...
        this.jq.off("keydown.tree blur.tree", ".ui-treenode-label").on("keydown.tree", ".ui-treenode-label", null, function(j) {
            ...
            switch (j.which) {
                ...
                case m.DOWN:
                    var f = null,
                        c = a.focusedNode.find("> ul > li:visible:first");
                    if (c.length) {
                        f = c
                    } else {
